...
> **2026-09-17 更新**：dy_obs 机制已删除（提交 `96f2d0b`），“保留于兼容链路”的前提不再成立。删除依据是一次静止场景实测：把 4.2 m 处的箱子在第 100 帧移除后，幽灵点无论 dy_obs 打开还是关闭都在 4 帧内消失。因此上文最后一句的回归项取消——新链路已无 dy_obs 可对齐。
...
只累计连续有效扫描覆盖的间隔；扫描中断、TF 失败、时间回跳、重复时间戳不能推进计时或伪造刷新。跨中断恢复时不补算整个停机时长。视野判据要取代的正是旧 `noDecayDis` 那种“距离近就永久豁免”的做法（该参数已于 2026-09-19 删除：实车配置本来就是 0，等于关闭）。
...
| D20 删除 dy_obs 是"纯减法、行为不变" | **撤回**（机制仍已删除） | dy_obs 删除不是行为不变 |
...
> **D20 的更正需限定范围**："不是行为不变"与"该删除已执行"并不矛盾。实测只覆盖了幽灵点清除这一项（静止场景，开关两态都是 4 帧），视场内快速横穿的目标未测；机制本身已于 2026-09-17 删除（提交 `96f2d0b`），若将来需要动态目标抑制，应按新需求重新设计，而不是恢复 dy_obs。
...
> 本节引用的源文件在 2026-09-17 的重构中已改名或移动：`terrain_processor.{hpp,cpp}` → `terrain_pipeline.{hpp,cpp}`，体素与采集两组阶段移入 `terrain_voxel_map.{hpp,cpp}`，dy_obs 相关函数已删除。文中的行号只对应上述快照，按行号找不到内容属正常。
