...
namespace bridge {
  ...
  public:  // 检测雷达是否连接
    inline bool isLidarConnected() {
      ...
      int point_port =
          config["MID360"]["lidar_net_info"]
                ["point_data_port"];  // 读取端口（建议检测 point_data_port）
  ...
  private:
    ...
    // 模板类桥接器
    std::shared_ptr<RosSerialBridge<geometry_msgs::msg::Twist>>
        bridge_twist_pc_;
    std::shared_ptr<RosSerialBridge<std_msgs::msg::Float32>> bridge_Yaw_mcu_;
    std::shared_ptr<RosSerialBridge<geometry_msgs::msg::Twist>>
        bridge_TESspeed_mcu_;
    std::shared_ptr<RosSerialBridge<geometry_msgs::msg::Point>>
