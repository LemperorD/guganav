  ...
  : Node("serial_driver_node", options)
{
  ...
  auto serial_cb_group = create_callback_group(
...
uint8_t* SerialDriverNode::encodeTwist(const geometry_msgs::msg::Twist& msg) {
  ...
  serial_driver_main_->writeFloatLE(&payload[1], static_cast<float>(angle_init_));

  payload[5] = true;

  serial_driver_main_->writeFloatLE(&payload[6], vx_Y);
  serial_driver_main_->writeFloatLE(&payload[10], vy_Y);

  serial_driver_main_->writeFloatLE(&payload[14], vx);
  serial_driver_main_->writeFloatLE(&payload[18], vy);
  serial_driver_main_->writeFloatLE(&payload[22], -wz);
...
geometry_msgs::msg::Twist SerialDriverNode::decodeTESspeed(const uint8_t* payload) {
  geometry_msgs::msg::Twist msg;
  msg.angular.z = static_cast<double>(serial_driver_main_->readFloatLE(&payload[3]));
  return msg;
}
...
inline double SerialDriverNode::dwa_filter(double sample) {
...
inline geometry_msgs::msg::Twist SerialDriverNode::transformVelocityToChassis(
...
void SerialDriverNode::publishRefereeData() {
  ...
  uint16_t current_hp, ammo17, heat1,
      heat2;  // 当前血量、剩余17mm弹量、热量1、热量2
...
pb_rm_interfaces::msg::RfidStatus SerialDriverNode::rfid2ros(uint32_t rfid) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(bridge::SerialDriverNode)