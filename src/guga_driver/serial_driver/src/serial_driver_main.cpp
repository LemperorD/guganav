...
SerialDriverMain::SerialDriverMain(std::string serial_port, int baud_rate)
  ...
  if (!serial_port_.empty()) {
    ...
    std::cout << YELLOW << "No serial port specified, auto-detecting..." << RESET << '\n';
    std::string port = findSerialPort();
    if (!port.empty()) {
      openSerialPort(port, baud_rate_);
    } else {
      std::cerr << RED << "No serial port found." << RESET << '\n';
    }
  }
...
void SerialDriverMain::openSerialPort(const std::string& port_name,
  ...
  if (fd_ == -1) {
    ...
    std::cerr << RED << "Failed to open serial port: "
              << strerror_r(saved_errno, errbuf.data(), errbuf.size())
              << RESET << '\n';
    return;
  }
  std::cout << GREEN << "Serial port opened: " << port_name << RESET << '\n';

  configureSerialPort(baud_rate);
  std::cout << GREEN << "Serial initialized: " << port_name << RESET << '\n';
}
...
std::string SerialDriverMain::findSerialPort() {
  ...
  if (ec) {
    std::cerr << RED << "Failed to enumerate /dev: " << ec.message() << RESET << '\n';
  }
  return "";
}
...
void SerialDriverMain::configureSerialPort(int baud_rate) {
  ...
  if (tcgetattr(fd_, &tty) != 0) {
    std::cerr << RED << "Failed to get serial attributes" << RESET << '\n';
    close(fd_);
    fd_ = -1;
    return;
  }
  ...
  switch (baud_rate) {
    ...
    case 230400:
      ...
      break;
    default:
      std::cerr << RED << "Unsupported baud rate: " << baud_rate << RESET << '\n';
      return;
  }
  ...
  if (tcsetattr(fd_, TCSANOW, &tty) != 0) {
    std::cerr << RED << "Failed to set serial attributes"
              << RESET << '\n';
    close(fd_);
    fd_ = -1;
    return;
...
void SerialDriverMain::sendDataFrame(const uint8_t* data, size_t len) const {
  std::cout << GREEN << "Sending data frame, len=" << len << RESET << '\n';
  const std::lock_guard<std::mutex> lock(serial_mutex_);
  if (fd_ < 0) {
    std::cerr << RED << "Serial port not available" << RESET << '\n';
    return;
  }
  ...
  if (total_written != frame_len) {
    const int saved_errno = errno;
    std::array<char, 256> errbuf{};
    std::cerr << RED << "TX failed " << total_written << "/"
              << frame_len << ": "
              << strerror_r(saved_errno, errbuf.data(), errbuf.size())
              << RESET << '\n';
  }
...
void SerialDriverMain::processBuffer() {
  ...
  while (buffer_index_ >= FRAME_MIN_SIZE
    ...
    if (!isSupportedCommand(cmd)) {
      std::cout << YELLOW << "Unknown CMD=0x" << std::hex
    ...
    if (frame_len > std::min(BUFFER_SIZE, MAX_FRAME_LEN)) {
      std::cout << RED << "Invalid frame length: " << frame_len << ", drop buffer" << RESET << '\n';
      buffer_index_ = 0;
      return;
    }
    ...
    if (calc != buffer_[frame_len - 1]) {
      std::cout << YELLOW << "CRC8 failed: calc=0x" << std::hex
...
void SerialDriverMain::processFrame(const uint8_t* frame) {
  ...
  std::cout << YELLOW << "Unknown CMD=0x" << std::hex
            ...
            << static_cast<int>(len) << " (ignored)" << RESET
            << '\n';
}
...
void SerialDriverMain::timerCallback() {
  ...
  if (buffer_index_ >= BUFFER_SIZE - 64) {
    std::cout << YELLOW << "Buffer near full, clearing" << RESET << '\n';
    buffer_index_ = 0;
  }
  ...
  if (n > 0) {
    ...
    if (buffer_index_ + static_cast<size_t>(n) > BUFFER_SIZE) {
      std::cout << RED << "Buffer overflow, drop" << RESET << '\n';
      buffer_index_ = 0;
      return;
    }
...
void SerialDriverMain::tryReconnect() {
  ...
  if (!serial_port_.empty()) {
    ...
    std::string port = findSerialPort();
    if (!port.empty()) {
      std::cout << GREEN << "Auto-detected serial port: " << port << RESET << '\n';
      openSerialPort(port, baud_rate_);
      std::cout << CYAN << "Serial reconnected: " << port << RESET << '\n';
    } else {
      std::cerr << RED << "waiting..." << RESET << '\n';
    }
  }
