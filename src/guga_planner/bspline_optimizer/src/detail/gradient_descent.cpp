...
namespace bspline_optimizer::detail {
  ...
  std::vector<double> gradientDescent(const std::vector<double>& init_params,
                                      const CostCache& cc, EndPoints& endpoints,
                                      Weight& weight, const float* esdf_gx,
                                      const float* esdf_gy, EsdfData& esdfdata,
                                      CostNormal& costnormal, int max_iters,
                                      double corridor_hw, bool& converged,
                                      int& iters_out) {
    ...
    auto normalise = [&](std::vector<double> gradient) {
      double norm = sqrt(std::inner_product(gradient.begin(), gradient.end(),
                                            gradient.begin(), 0.0));
      return norm;
    };
    computeGradient(x, cc, endpoints, weight, esdfdata, costnormal, grad,
                    esdf_gx, esdf_gy);

    double f_best = evalCost(x, cc, endpoints, weight, esdfdata, costnormal);

    auto evalclamped = [&](const std::vector<double>& x_try,
                           double& cost_clamped) {
      ...
      cost_clamped = evalCost(x_clamped, cc, endpoints, weight, esdfdata,
                              costnormal);
      return x_clamped;
    };
    ...
    for (; iter < max_iters; ++iter) {
      ...
      for (int ls = 0; ls < 30; ++ls) {
        for (int i = 0; i < N; ++i) {
          x_try[static_cast<size_t>(i)] =
              ...
              + alpha * direction[static_cast<size_t>(i)];
        }
        double f_try{};
      ...
      computeGradient(x, cc, endpoints, weight, esdfdata, costnormal, g_new,
                      esdf_gx, esdf_gy);
      ...
      for (int i = 0; i < N; ++i) {
        direction[static_cast<size_t>(i)] =
            -grad[static_cast<size_t>(i)]
            + beta * direction[static_cast<size_t>(i)];
      }
