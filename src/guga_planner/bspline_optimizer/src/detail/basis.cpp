...
namespace bspline_optimizer::detail {
  ...
  int BasisCache::findSpan(double u, const Eigen::RowVectorXd& knots, int n,
                           int p) {
    if (u >= knots(n + 1)) {
      return n;
    }
  ...
  void BasisCache::basisDerivsAt(double u, const Eigen::RowVectorXd& knots,
                                 int p, int M, std::vector<double>& N,
    ...
    for (int j = 1; j <= p; ++j) {
      ...
      for (int r = 0; r < j; ++r) {
        ...
        const double temp = ndu[static_cast<size_t>(r) * (p + 1) + (j - 1)]
                            / ndu[static_cast<size_t>(j) * (p + 1) + r];
    ...
    for (int j = 0; j <= p; ++j) {
      ...
      if (i >= 0 && i < M) {
        N[static_cast<size_t>(i)] = ndu[static_cast<size_t>(j) * (p + 1) + p];
      }
    }
    ...
    for (int r = 0; r <= p; ++r) {
      ...
      for (int k = 1; k <= korder; ++k) {
        ...
        for (int j = j1; j <= j2; ++j) {
          find_index(
              s2, j) = (find_index(s1, j) - find_index(s1, j - 1))
                       / ndu[static_cast<size_t>(pk + 1) * (p + 1) + (rk + j)];
          d += find_index(s2, j)
               * ndu[static_cast<size_t>(rk + j) * (p + 1) + pk];
        }
        ...
        if (i >= 0 && i < M) {
          if (k == 1) {
            ...
            N2[static_cast<size_t>(i)] = static_cast<double>(p * (p - 1)) * d;
          }
        }
        std::swap(s1, s2);
      }
