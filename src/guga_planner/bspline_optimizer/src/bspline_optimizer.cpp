...
#include "bspline_optimizer/detail/gradient_descent.hpp"
#include "bspline_optimizer/detail/grid_utils.hpp"

namespace bspline_optimizer {

  using namespace bspline_optimizer::detail;
  ...
  bool BSplineOptimizer::fit(
    ...
    if (state_.total_arc_length < 1e-9) {
      return false;
    }
    ...
    using SplineFitter = Eigen::SplineFitting<Eigen::Spline<double, 2, 7>>;
    ...
    for (int i = 0; i < M; ++i) {
      ...
      double target_arc_frac = static_cast<double>(i)
  ...
  void BSplineOptimizer::rebuildSpline() {
    if (state_.control_points.cols() < state_.effective_degree + 1) {
      return;
    }
  ...
  std::vector<std::pair<double, double>> BSplineOptimizer::sample(int N) const {
    ...
    if (!spline_) {
      ...
      if (state_.original_points.empty()) {
        return out;
      }
      out.reserve(N);
  ...
  BSplineResult BSplineOptimizer::optimize(int num_samples) {
    ...
    if (config_.enable_gradient_descent) {
      ...
      evalTerms(params, cc, endpoints, esdfdata, costnormal.js0, costnormal.jd0,
                costnormal.je0);

      if (costnormal.js0 < 1e-12) {
        costnormal.js0 = 1.0;
      }
      if (costnormal.jd0 < 1e-12) {
        costnormal.jd0 = 1.0;
      }
      if (costnormal.je0 < 1e-12) {
        costnormal.je0 = 1.0;
      }
      auto opt = gradientDescent(
          params, cc, endpoints, weight, state_.esdf_gradient_x,
          state_.esdf_gradient_y, esdfdata, costnormal, config_.max_iterations,
          config_.corridor_halfwidth, converged, iters_out);
    ...
    if (state_.costmap_data != nullptr) {
      for (auto& p : path) {
        projectPointToFree(state_.costmap_data, state_.costmap_w,
                           state_.costmap_h, p.first, p.second);
      }
      ...
      for (int pass = 0; pass < 8; ++pass) {
        ...
        for (size_t i = 0; i < path.size(); ++i) {
          ...
          for (int s = 1; s < steps; ++s) {
            ...
            auto pt = std::make_pair(x0 + t * (x1 - x0), y0 + t * (y1 - y0));
            if (inObstacle(state_.costmap_data, state_.costmap_w,
                           state_.costmap_h, pt.first, pt.second)) {
              projectPointToFree(state_.costmap_data, state_.costmap_w,
                                 state_.costmap_h, pt.first, pt.second);
              projected_any = true;
            }
    ...
    for (int i = 0; i < M; ++i) {
      result.control_points_xy.emplace_back(state_.control_points(0, i),
                                            state_.control_points(1, i));
    }
    return result;
  }
