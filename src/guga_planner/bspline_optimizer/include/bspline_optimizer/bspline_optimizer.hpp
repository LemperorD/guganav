...
namespace bspline_optimizer {
  ...
  struct BSplineConfig {
    ...
    double esdf_safe_distance{
        0.3};  // ESDF safe distance (m, min dist from path to obstacle)
  ...
  public:
     ...
     * @return 拟合成功返回 true。
     */
    [[nodiscard]] bool fit(const std::vector<std::pair<double, double>>& path);
    ...
    /** @brief fit() 是否成功。 */
    [[nodiscard]] bool isFitted() const {
      return fitted_;
    }

    /** @brief 访问内部状态 (只读 / 可变)。 */
    [[nodiscard]] const BSplineState& state() const {
      return state_;
    }
    [[nodiscard]] BSplineState& state() {
      return state_;
    }
    [[nodiscard]] const BSplineConfig& config() const {
      return config_;
    }
