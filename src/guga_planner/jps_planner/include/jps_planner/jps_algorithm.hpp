...
class JPSAlgorithm {
  ...
  private:
    ...
    static constexpr unsigned char MAX_NON_OBSTACLE =252;  // 最高非障碍物代价值
    ...
    static bool isObstacle(const JPSConfig& c, const JPSState& s,
                                         int x, int y);
    bool canStep(const JPSConfig& c, const JPSState& s, int x, int y, 
                 int dx,int dy);
    static bool withinLimits(const JPSState& s, int x, int y){
      return x >= 0 && x < s.size_x && y >= 0 && y < s.size_y;
    };
    static bool isTraversableCell(const JPSConfig& c,const JPSState& s, int x, int y) {
    ...
    bool hasForcedNeighborHoriz(const JPSConfig& c,const JPSState& s,
                               int x, int y,int dx);
    ...
    bool hasForcedNeighbor(const JPSConfig& c, const JPSState& s, int x, int y,
                           int dx, int dy);
    void pruneNeighbors(const JPSConfig& c, const JPSState& s, 
...
public:
  ...
  static detourCornerHuggingDiagonals(
