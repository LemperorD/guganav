 ...
 * 本文件是前端的核心搜索模块，提供:
 * - A*: 经典的启发式图搜索，逐格扩展
 * - JPS (Jump Point Search): 针对均匀栅格地图的高效路径搜索算法
 *   通过"跳跃"跳过直线路径上的冗余节点，仅在实际改变方向的"跳点"处分支
 *
 * JPS 算法的核心原理:
 * 1. 邻居剪枝: 根据搜索方向，过滤掉能被更短路径到达的邻居节点
 * 2. 强制邻居: 当某些邻居因为障碍物而无法被更优路径到达时，强制检查这些点
 * 3. 递归跳跃: 沿直线方向一直前进，直到遇到障碍物、强制邻居或目标点
 *
 * 数据结构设计:
 * - State: 图搜索中的节点，存储栅格坐标、方向和A*相关代价
 * - JPS2DNeib/JPS3DNeib: 预计算的邻居查找表（LUT），存储每个方向对应的自然邻居和强制邻居
 * - priorityQueue: 基于 boost::d_ary_heap 的可变二叉堆优先队列，支持 O(log n) 的增删改操作
 */
