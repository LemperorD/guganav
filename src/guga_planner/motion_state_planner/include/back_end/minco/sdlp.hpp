 ...
 * 则将问题投影到该约束的边界上递归求解。
 *
 * 复杂度：O(d! * m) 期望时间，其中 d 是变量维度，m 是约束数。
 * 在 d << m 时极为高效。
 *
 * 本文件包含：
 * - linfracprog<d>: 递归模板，求解 d 维线性分式规划
 *   minimize dot(n,x) / dot(d,x)  s.t. Ax >= 0
 *   (通过 C++ 模板元编程展开到基础情况 d=1)
 *
 * - linprog<d>: 标准线性规划的 Eigen 接口
 *   minimize c^T x  s.t. Ax <= b
 *   (通过添加齐次坐标转化为线性分式规划)
 *
 * - wedge / lp_base_case: 2D 基础情况的可行域求解
 *
 * 在本项目中的用途：
 *   通过线性规划将曲率约束转化为速度/角速度的线性约束多边形，
