...
float esdfDistanceAt(
  ...
  double wx, double wy)
{
  if (!esdfdata.esdf_dist) {return static_cast<float>(esdfdata.esdf_max);}
  ...
  if (ix < 0 || ix + 1 >= esdfdata.esdf_w || iy < 0 || iy + 1 >= esdfdata.esdf_h) {
  ...
  size_t idx00 = static_cast<size_t>(iy) * static_cast<size_t>(esdfdata.esdf_w) +
  ...
  double & gx, double & gy)
{
  ...
  if (ix < 0 || ix + 1 >= esdfdata.esdf_w || iy < 0 || iy + 1 >= esdfdata.esdf_h) {return;}
