...
`bspline_opt` 不是 ROS 节点，而是一个纯 C++ 库。公开 API 在
`bspline_optimizer.hpp` / `bspline_optimizer.cpp`，代价函数、梯度下降、栅格查询等
内部实现拆到 `src/detail/` 与 `include/bspline_opt/detail/`。
...
`bspline_opt` 不订阅 ROS Topic，通过函数调用传参。
...
```bash
# 构建并运行 bspline_opt 测试
colcon build --packages-select bspline_opt
colcon test --packages-select bspline_opt --event-handlers console_direct+

# 直接运行联合测试（JPS 前端数据 → B-spline 后端）
./build/bspline_opt/joint_test
```
