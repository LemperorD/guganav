...
def generate_launch_description():
    ...
    #  decision_simple enable + params =====
    enable_decision_simple = LaunchConfiguration("enable_decision_simple")
    decision_simple_params_file = LaunchConfiguration("decision_simple_params_file")
    ...
    #  decision_simple args 
    declare_enable_decision_simple_cmd = DeclareLaunchArgument(
        "enable_decision_simple",
        default_value="True",
        description="Whether to start decision_simple",
    )

    declare_decision_simple_params_cmd = DeclareLaunchArgument(
        "decision_simple_params_file",
        default_value=os.path.join(decision_simple_dir, "config", "decision_simple.yaml"),
        description="Full path to decision_simple params yaml",
    )
    ...
    decision_simple_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(decision_simple_dir, "launch", "decision_simple.launch.py")
        ),
        condition=IfCondition(enable_decision_simple),
        launch_arguments={
            "namespace": namespace,
            "params_file": decision_simple_params_file,
        }.items(),
    )
    ...
    # decision_simple args register 
    ld.add_action(declare_enable_decision_simple_cmd)
    ld.add_action(declare_decision_simple_params_cmd)
    ...
    #  start decision_simple with the same namespace
