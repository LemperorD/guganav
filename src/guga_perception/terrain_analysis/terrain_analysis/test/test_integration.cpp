// 集成测试：只通过 ROS 话题驱动节点，不直接调用 ingest* / runStages /
// 体素地图。
//
// 覆盖的是单元测试看不到的那一层：话题订阅与消息转换（Odometry → 位姿、
// PointCloud2 → PCL 点云与时间戳）、节点的逐帧数据分发（裁剪点云与雷达位姿
// 交给体素地图，采集结果交回管线）、以及发布出去的消息本身（frame_id、
// stamp、intensity 语义）。
//
