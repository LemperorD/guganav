...
/**
 * @brief 网格：两张固定尺寸网格的类型与换算工具。
 *
 * 前半段的 PersistentVoxelGrid 累积点云并随车滚动，后半段的 PerFrameHeightGrid
 * 逐帧存地面高度；两者的布局约定相同（行主序，线性下标 = 宽度 × 行 + 列），
 * 由各自的 linearIndex 提供。其余函数都是"格 ←→ 坐标"与"格 ←→ 点/邻域"的纯
 * 换算，不持有状态，两半与测试都可以直接用。
 *
 * 命名空间沿用本包的约定：数据结构（两个网格类型，与 PersistentVoxelConfig
 * 等一样） 位于全局命名空间，算法与工具函数位于 terrain_analysis 内。
 */
