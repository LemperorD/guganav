...
namespace terrain_analysis {
  ...
  class FrameReceiveTest : public testing::Test {
  protected:
    // 配置先声明、map 后声明：map 只存配置的常量引用，故配置必须活得更久
    // （析构顺序相反：map 先销毁）。
    /** @brief 配置由测试持有，构造时注入。 */
    PersistentVoxelConfig config_;
    TestVoxelMap voxel_map_{config_};
  };

  // 默认构造后没有待处理帧，也没有记下任何雷达位置
  TEST_F(FrameReceiveTest, DefaultState_NoPendingFrame) {
    EXPECT_FALSE(voxel_map_.hasPendingFrame());
    EXPECT_TRUE(voxel_map_.frameCloud().points.empty());
  }

  // 接一帧后记下雷达位置、帧时刻，并标记有帧待处理
  TEST_F(FrameReceiveTest, Receive_StoresLidarPositionAndTime) {
    auto cloud = MakeCloud(0, 0, 0);
    voxel_map_.receiveFrame(*cloud, {1.0, 2.0, 3.0}, 100.0);
  ...
  TEST_F(FrameReceiveTest, Receive_FirstCall_SetsInitTime) {
    auto cloud = MakeCloud(0, 0, 0);
    voxel_map_.receiveFrame(*cloud, {0.0, 0.0, 0.0}, 100.0);

    EXPECT_DOUBLE_EQ(voxel_map_.initTime(), 100.0);
    EXPECT_DOUBLE_EQ(voxel_map_.elapsedSeconds(), 0.0);
  }

  // 观测时刻写进 intensity：体素叶靠它判年龄，首帧为 0、第二帧为两帧之差
  TEST_F(FrameReceiveTest, Receive_StampsObservationTimeIntoIntensity) {
    auto cloud = MakeCloud(0, 0, 0);
    voxel_map_.receiveFrame(*cloud, {0.0, 0.0, 0.0}, 100.0);
    ASSERT_EQ(voxel_map_.frameCloud().points.size(), 1U);
    EXPECT_FLOAT_EQ(voxel_map_.frameCloud().points[0].intensity, 0.0F);

    voxel_map_.receiveFrame(*MakeCloud(0, 0, 0), {0.0, 0.0, 0.0}, 100.5);
    ASSERT_EQ(voxel_map_.frameCloud().points.size(), 1U);
  ...
  // 超出体素网格接收范围的点被裁剪掉
  TEST_F(FrameReceiveTest, Receive_FiltersPointsBeyondVoxelRange) {
  ...
  TEST_F(FrameReceiveTest, Update_ClearsPendingFlag) {
    auto cloud = MakeCloud(0, 0, 0);
    voxel_map_.receiveFrame(*cloud, {0.0, 0.0, 0.0}, 100.0);
    voxel_map_.update();
