...
registered_scan_publisher = create_publisher<sensor_msgs::msg::PointCloud2>("/registered_scan", 1000);
        ...
        small_point_lio->set_odometry_callback([this, lidar_frame](const common::Odometry &odometry) {
            ...
            try {
                base_link_to_lidar_frame_transform = tf_buffer->lookupTransform(lidar_frame, "base_footprint", time_msg);
        ...
        small_point_lio->set_pointcloud_callback([this, save_pcd, lidar_frame](const std::vector<Eigen::Vector3f> &pointcloud) {
            if (pointcloud_publisher->get_subscription_count() > 0) {
                ...
                try {
                    lidar_frame_to_base_link_transform = tf_buffer->lookupTransform("base_footprint", lidar_frame, time_msg);
                } catch (tf2::TransformException &ex) {
