...
namespace faster_lio {
  ...
  template <>
  inline Eigen::Matrix<float, 3, 1> ToEigen<float, 3, pcl::PointXYZ>(
      const pcl::PointXYZ& pt) {
    return pt.getVector3fMap();
  }

  template <>
  inline Eigen::Matrix<float, 3, 1> ToEigen<float, 3, pcl::PointXYZI>(
  ...
  public:
    ...
    IVoxNode() = default;
    IVoxNode(const PointT&, float) {}  /// same with phc
  ...
  struct IVoxNodePhc<PointT, dim>::PhcCube {
    ...
    PhcCube(uint32_t index, const PointT& pt) : idx(index) {
      mean.add(pt);
    }
  ...
  IVoxNodePhc<PointT, dim>::IVoxNodePhc(const PointT& center,
    ...
    assert(phc_order <= 8);
    phc_side_length_inv_ = std::pow(2, phc_order_) / side_length;
    min_cube_ = center.getArray3fMap() - side_length / 2.0;
  ...
  int IVoxNodePhc<PointT, dim>::KNNPointByCondition(
    ...
    const int max_search_cube_side_length = std::pow(
        2, std::ceil(std::log2(max_range * phc_side_length_inv_)));
    const auto max_search_idx_th = static_cast<uint32_t>(
        8 * max_search_cube_side_length * max_search_cube_side_length
        * max_search_cube_side_length);
    const auto max_neighbors = static_cast<std::size_t>(std::max(K, 0));
    ...
    while (!forward_reach_boundary() && !backward_reach_boundary()) {
      ...
      if (dis_points.size() > max_neighbors) {
        break;
      }
    }

    if (forward_reach_boundary()) {
      while (!backward_reach_boundary()
             && dis_points.size() < max_neighbors) {
        dis_points.emplace_back(create_dist_point(backward_it.base()));
        backward_it++;
      }
    }
    ...
    if (backward_reach_boundary()) {
      while (!forward_reach_boundary() && dis_points.size() < max_neighbors) {
        dis_points.emplace_back(create_dist_point(forward_it));
        forward_it++;
      }
    }
  ...
  uint32_t IVoxNodePhc<PointT, dim>::CalculatePhcIndex(const PointT& pt) const {
    Eigen::Matrix<float, dim, 1> eposf = (pt.getVector3fMap() - min_cube_)
                                         * phc_side_length_inv_;
