...
两种 profile 均使用 `base_footprint`，不需要 `gimbal_yaw_fake`。PID profile
支持通过 `chassis_mode=2` 和 `cmd_spin` 输出目标 `wz`；MPPI profile 只使用
Nav2 自带 critics，不依赖额外的 Guga critic 包。输出继续经过
