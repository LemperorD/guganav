...
void OmniPidPursuitControllerNode::configure(
  ...
  node->get_parameter(plugin_name_ + ".v_linear_max",
                      config_.v_linear_max);
  node->get_parameter(plugin_name_ + ".v_linear_min",
                      config_.v_linear_min);
  node->get_parameter(plugin_name_ + ".v_angular_max",
                      config_.v_angular_max);
  node->get_parameter(plugin_name_ + ".v_angular_min",
                      config_.v_angular_min);
  node->get_parameter(plugin_name_ + ".max_robot_pose_search_dist",
                      config_.max_robot_pose_search_dist);
  node->get_parameter(plugin_name_ + ".curvature_min",
                      config_.curvature_min);
  node->get_parameter(plugin_name_ + ".curvature_max",
                      config_.curvature_max);
...
OmniPidPursuitControllerNode::computeVelocityCommands(
  ...
  for (int i = 0; i < sample_points; ++i) {
    ...
    transformPose(costmap_ros_->getGlobalFrameID(),
                  transformed_plan.poses[static_cast<size_t>(index)], map_pose);
