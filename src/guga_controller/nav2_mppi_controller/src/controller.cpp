...
namespace nav2_mppi_controller {
  ...
  geometry_msgs::msg::TwistStamped MPPIController::computeVelocityCommands(
    ...
    if (clock_->now() - last_time_called_
        > rclcpp::Duration::from_seconds(reset_period_)) {
      reset();
    }
    last_time_called_ = clock_->now();

    std::lock_guard<std::mutex> param_lock(*parameters_handler_->getLock());
    nav_msgs::msg::Path transformed_plan = path_handler_.transformPath(
        robot_pose);
    ...
    geometry_msgs::msg::TwistStamped cmd = optimizer_.evalControl(
        robot_pose, robot_speed, transformed_plan, goal_checker);
...
PLUGINLIB_EXPORT_CLASS(nav2_mppi_controller::MPPIController,
