#!/usr/bin/env python3
"""
plot_omni_results.py — 读取 test_omni_solver 输出的 CSV 数据并绘制图表。

用法:
    cd test_model_cc/build && ./test_omni_solver
...
def plot_open_loop(data_dir, out_dir):
    ...
    fig.suptitle("Open-Loop Model: Omni RK4 (vx=%.1f, vy=%.1f, ω=%.1f)" %
                 (p.get("u_vx", 0), p.get("u_vy", 0), p.get("u_omega", 0)),
                 fontsize=13)
...
def plot_closed_loop(data_dir, out_dir, tag):
    ...
    fig.suptitle("MPC Closed-Loop (Omni): " + titles.get(tag, tag), fontsize=13)
    ...
    # 状态 (px, py, theta)
    state_axes = [axes[0, 1], axes[0, 2], axes[1, 0]]
    for i, lab in enumerate(["px [m]", "py [m]", "theta [rad]"]):
        ax = state_axes[i]
    ...
    # 控制 (vx, vy, omega)
    ctrl_axes = [axes[1, 1], axes[1, 2], axes[2, 0]]
    for i, lab in enumerate(["vx [m/s]", "vy [m/s]", "ω [rad/s]"]):
        ax = ctrl_axes[i]
        ax.plot(t_ctrl, ud[:, i], "b-", lw=1.0, label=lab)
        bound = 2.5 if i < 2 else 4.0
        ax.axhline(bound, color="r", ls="--", alpha=0.4, lw=0.8)
        ax.axhline(-bound, color="r", ls="--", alpha=0.4, lw=0.8)
        ax.set_xlabel("time [s]"); ax.set_ylabel(lab)
        ax.set_title("Control: " + lab); ax.legend(fontsize=8); ax.grid(True)
...
def plot_solve_times(data_dir, out_dir):
    ...
    fig.suptitle("MPC Solve Time Distribution (Omni)", fontsize=13)
...
def plot_error_over_time(data_dir, out_dir):
    ...
    tags = ["circle", "figure8"]
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    fig.suptitle("Tracking Error Over Time (Omni)", fontsize=13)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        data_dir = os.path.join(script_dir, "data_omni")
    ...
    if not csvs:
        print(f"No CSV files in {data_dir}. Run ./test_omni_solver first.")
        sys.exit(0)

    print("=" * 60)
    print("Plotting acados Omni MPC Simulation Results")
