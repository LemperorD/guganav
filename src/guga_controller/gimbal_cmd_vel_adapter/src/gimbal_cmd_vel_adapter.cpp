...
#include "gimbal_cmd_vel_adapter/gimbal_cmd_vel_adapter.hpp"
...
GimbalCmdVelAdapter::GimbalCmdVelAdapter(const rclcpp::NodeOptions & options)
: Node("gimbal_cmd_vel_adapter", options)
{
  RCLCPP_INFO(get_logger(), "Start GimbalCmdVelAdapter!");

  this->declare_parameter<std::string>("robot_base_frame", "gimbal_link");
  this->declare_parameter<std::string>("nav2_robot_base_frame", "gimbal_link_fake");
  ...
  this->get_parameter("nav2_robot_base_frame", nav2_robot_base_frame_);
  ...
  cmd_spin_sub_ = this->create_subscription<example_interfaces::msg::Float32>(
    cmd_spin_topic_, 1,
    std::bind(&GimbalCmdVelAdapter::cmdSpinCallback, this, std::placeholders::_1));
  cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
    input_cmd_vel_topic_, 10,
    std::bind(&GimbalCmdVelAdapter::cmdVelCallback, this, std::placeholders::_1));

  chassis_mode_sub_ = this->create_subscription<std_msgs::msg::UInt8>(
    chassis_mode_topic_, 1,
    std::bind(&GimbalCmdVelAdapter::chassisModeCallback, this, std::placeholders::_1));
  ...
  odom_sub_filter_.registerCallback(
    std::bind(&GimbalCmdVelAdapter::odometryCallback, this, std::placeholders::_1));
  local_plan_sub_filter_.registerCallback(
    std::bind(&GimbalCmdVelAdapter::localPlanCallback, this, std::placeholders::_1));
  ...
  sync_->registerCallback(std::bind(
    &GimbalCmdVelAdapter::syncCallback, this, std::placeholders::_1, std::placeholders::_2));

  tf_sub_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(50), std::bind(&GimbalCmdVelAdapter::updateGimbalYaw, this));

  // 50Hz Timer to send transform from `robot_base_frame` to `nav2_robot_base_frame`
  tf_pub_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(20), std::bind(&GimbalCmdVelAdapter::publishTransform, this));
}

void GimbalCmdVelAdapter::chassisModeCallback(const std_msgs::msg::UInt8::SharedPtr msg)
{
  chassis_mode_ = msg->data;
}

void GimbalCmdVelAdapter::cmdSpinCallback(const example_interfaces::msg::Float32::SharedPtr msg)
{
  spin_speed_ = msg->data;
}

void GimbalCmdVelAdapter::odometryCallback(const nav_msgs::msg::Odometry::ConstSharedPtr & msg)
{
...
void GimbalCmdVelAdapter::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
...
void GimbalCmdVelAdapter::localPlanCallback(const nav_msgs::msg::Path::ConstSharedPtr & /*msg*/)
{
  // Consider nav2_controller_server is activated when receiving local_plan
  last_controller_activate_time_ = rclcpp::Clock().now();
}

void GimbalCmdVelAdapter::syncCallback(
...
void GimbalCmdVelAdapter::updateGimbalYaw()
{
  try {
...
void GimbalCmdVelAdapter::publishTransform()
{
  geometry_msgs::msg::TransformStamped t;
  t.header.stamp = this->get_clock()->now();
  t.header.frame_id = robot_base_frame_;
  t.child_frame_id = nav2_robot_base_frame_;
...
geometry_msgs::msg::Twist GimbalCmdVelAdapter::transformVelocity(
  const geometry_msgs::msg::Twist::SharedPtr & twist, float yaw_diff)
{
  ...
  const double angular_velocity =
    chassis_mode_ == chassisFollowed ? twist->angular.z : twist->angular.z + spin_speed_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(gimbal_cmd_vel_adapter::GimbalCmdVelAdapter)
